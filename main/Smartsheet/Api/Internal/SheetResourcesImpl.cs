...
namespace Smartsheet.Api.Internal
{
	...
	public class SheetResourcesImpl : AbstractResources, SheetResources
	{
		...
		/// <exception cref="ServiceUnavailableException"> if the REST API service is not available (possibly due To rate limiting) </exception>
		/// <exception cref="SmartsheetException"> if there is any other error during the operation </exception>
		public virtual Sheet GetSheet(long sheetId, IEnumerable<SheetInclusion> includes, IEnumerable<ObjectExclusion> excludes, IEnumerable<long> rowIds, IEnumerable<long> rowNumbers, IEnumerable<long> columnIds, long? pageSize, long? page)
		{
		...
		{
			return this.CreateResource("sheets?include=" + QueryUtil.GenerateCommaSeparatedList<ObjectInclusion>(includes), typeof(Sheet), sheet);
		}
